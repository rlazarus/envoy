  ...
  for (auto& cluster : server_.clusterManager().clusters()) {
    ...
    for (auto& host : cluster.second.get().hosts()) {
      ...
      response.add(fmt::format("{}::{}::health_flags::{}\n", cluster.second.get().info()->name(),
                               host->address()->asString(), Upstream::HostUtility::healthFlagsToString(*host)));
...
Network::Address::InstancePtr AdminImpl::localAddress() { return server_.localInfo().address(); }
